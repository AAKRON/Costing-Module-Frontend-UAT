...
export default () => {
  ...
  const OptionWithSubmenu = ({ icon, label, nameMenu, targetMenu, onTargetMenu, options }) => (
    <div>
      <MenuItem
        className='link'
  ...
  return (
    <Menu className='menu'>
      {Option({ icon: <DashboardIcon />, to: '/', label: 'Dashboard' })}
      <Divider style={{ margin: '0' }} />

      {Option({ icon: <FileDownloadRounded />, to: '/spreadsheet', label: 'Spreadsheet' })}
      <Divider style={{ margin: '0' }} />

      {OptionWithSubmenu({
        icon: <JobIcon />, label: 'Jobs', nameMenu: 'menu-jobs',
        targetMenu: menuJobs, onTargetMenu: setMenuJobs,
        options: [{ to: 'job_listings', label: 'Listing' }, { to: 'screens', label: 'Screen' }],
      })}
      <Divider style={{ margin: '0' }} />

      {OptionWithSubmenu({
        icon: <BlankIcon />, label: 'Blanks', nameMenu: 'menu-blanks',
        targetMenu: menuBlanks, onTargetMenu: setMenuBlanks,
        options: [
      ...
      {OptionWithSubmenu({
        icon: <ItemIcon />, label: 'Items', nameMenu: 'menu-items',
        targetMenu: menuItems, onTargetMenu: setMenuItems,
        options: [
      ...
      {OptionWithSubmenu({
        icon: <BomIcon />, label: 'Bill of Materials', nameMenu: 'menu-bill-materials',
        targetMenu: menuBillMaterials, onTargetMenu: setMenuBillMaterials,
        options: [
      ...
      {OptionWithSubmenu({
        icon: <RawMaterialIcon />, label: 'Raw Material', nameMenu: 'menu-raw-material',
        targetMenu: menuRawMaterial, onTargetMenu: setMenuRawMaterial,
        options: [
      ...
      {Option({ icon: <CostCalcIcon />, to: '/cost_calculator/create', label: 'Cost Calculator' })}
      <Divider style={{ margin: '0' }} />

      {isAdmin() && (
        <div>
          {OptionWithSubmenu({
            icon: <AdminIcon />, label: 'Admin', nameMenu: 'menu-admin',
            targetMenu: menuAdmin, onTargetMenu: setMenuAdmin,
            options: [
