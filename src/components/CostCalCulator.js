...
export default class CostCalCulator extends React.Component {
  ...
  componentDidMount() {
    axios.all([this.fetchJobs()]).then(
      axios.spread((job) => {
        this.setState({
          jobs: job.data.map((job) => `${job.job_number} - ${job.description}`),
        });
      })
    );
  }
  ...
  handleEditBlank = (blankIndex) => () => {
    this.setState({
      blank: { ...this.state.blanks[blankIndex], blankIndex: blankIndex },
    });
    this.handleBlankDialogOpen();
  };

  handleRemoveBlank = (blankIndex) => () => {
    var blanks = this.state.blanks.filter(
      (blank, index) => blankIndex !== index
    );
    this.setState({
      blanks: blanks,
    });
  ...
  handleBlankTextFieldChange = (event, value) => {
    ...
    this.setState({
      blank: { ...this.state.blank, [event.target.name]: value },
    });
  };
  ...
  handleEditJob = (jobIndex) => () => {
    this.setState({
      job: { ...this.state.selected_jobs[jobIndex], jobIndex: jobIndex },
    });
    this.handleJobDialogOpen();
  };

  handleRemoveJob = (jobIndex) => () => {
    var selected_jobs = this.state.selected_jobs.filter(
      (job, index) => jobIndex !== index
    );
    var screens = selected_jobs
  ...
  validateJob = () => {
    if (
      ...
      this.setState({
        error_job: {
          job_listing_id: requiredMessage,
          hour_per_piece: requiredMessage,
        },
      });
      return false;
    } else if (
    ...
    return (
      <TableRow key={blankIndex}>
        ...
        <TableRowColumn style={{ paddingLeft: 0 }}>
          <IconButton
            onClick={this.handleEditBlank(blankIndex)}
            style={buttonStyle}
          >
            <ContentCreate />
          </IconButton>
          <IconButton
            onClick={this.handleRemoveBlank(blankIndex)}
            style={buttonStyle}
          >
            <DeleteIcon />
          </IconButton>
  ...
  jobField = (job, jobIndex) => {
    return (
      <TableRow key={jobIndex}>
        ...
        <TableRowColumn style={textCenterAlign}>
          {job.hour_per_piece}
        </TableRowColumn>
        <TableRowColumn style={textRightAlign}>
        ...
        <TableRowColumn style={{ paddingLeft: 0 }}>
          <IconButton
            onClick={this.handleEditJob(jobIndex)}
            style={buttonStyle}
          >
            <ContentCreate />
          </IconButton>
          <IconButton
            onClick={this.handleRemoveJob(jobIndex)}
            style={buttonStyle}
          >
            <DeleteIcon />
          </IconButton>
            ...
            <TableBody displayRowCheckbox={false}>
              <TableRow>
                <TableRowColumn
                  ...
                  rowSpan={this.state.blanks.length + 2}
                >
                  Blanks
                </TableRowColumn>
              ...
              <TableRow>
                <TableRowColumn
                  style={rightColumnBorder}
                  rowSpan={this.state.selected_jobs.length + 2}
                >
                  Jobs
                </TableRowColumn>
              ...
              {typeof this.state.screens === 'object' &&
                this.state.screens.length > 0 && (
                  <TableRow>
                    <TableRowColumn
                      rowSpan={this.state.screens.length + 1}
                      style={rightColumnBorder}
                    >
                      Screens
                    </TableRowColumn>
                    <TableHeaderColumn colSpan={6}>Job</TableHeaderColumn>
                    <TableHeaderColumn colSpan={2} style={textCenterAlign}>
                      Screen Name
                    </TableHeaderColumn>
                  </TableRow>
                )}
              ...
              <TableRow>
                <TableRowColumn style={rightColumnBorder}>
                  Ink Cost
                </TableRowColumn>
                <TableRowColumn colSpan={2} style={textCenterAlign}>
                  <TextField
